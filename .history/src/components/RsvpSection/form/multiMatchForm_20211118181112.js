...
const MultiMatchForm = ({ potentialParties, onChooseParty }) => {
	...
	const onNext = () => {
		const chosenPartyValue = potentialParties.find((party) => party.id === chosenParty);
	...
	return (
		<div>
			<Title>
				We have found more than one match in the guest list. Please select the correct option from the list below.
			</Title>

			<RadioGroup label='Options' options={options} input={{ value: chosenParty, onChange: onChosenPartyChange }} />

			<ButtonGroup right>
				<Button buttonStyle={STYLES.PRIMARY} disabled={!chosenParty} onClick={onNext}>
					Next
				</Button>
			</ButtonGroup>
		</div>
