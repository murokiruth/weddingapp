...
class GuestsForm extends Component {
	...
	updateGuestChoices = (values) => {
		...
		const alreadyUpdatedIdx = currentUpdatedGuests.findIndex((updated) => updated.name === updatedGuest.name);
	...
	renderGuestForm = () => {
		...
		const initialGuestValues = updatedGuests[currentGuestIdx] || guests[currentGuestIdx] || {};
