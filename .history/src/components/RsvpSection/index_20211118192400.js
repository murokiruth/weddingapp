...
// `;
// const RetryButton = ({ close }) => <RetryContent onClick={close}>Retry</RetryContent>;
...
// 				const loadingPartiesErrorId = alerts.showError('An error has occurred. Please try again.', {
// 					closeButton: RetryButton,
// 					onClose: () => {
// 						this.setState({ loadingPartiesErrorId: null }, () => this.loadParties());
// 					},
// 				});
// 				this.setState({
...
// 			});
// 			alerts.showSuccess('You have already responded. Please find your details below.');
// 		} else if (foundParty) {
// 			if (partyNotFoundAlert) {
// 				alerts.close(partyNotFoundAlert);
// 			}
...
// 	renderNameForm = () => {
// 		return <NameForm onSubmit={this.onNameFormSubmit} onCancel={this.goToHome} />;
// 	};

// 	shouldRenderNameForm = () => {
// 		const { allParties, chosenParty, potentialParties, isLoading } = this.state;

// 		return allParties && allParties.length && !chosenParty && !potentialParties && !isLoading;
// 	};
...
// 		const partyRef = db.ref(`parties/${updatedParty.id}`);
// 		partyRef.set(updatedParty, (error) => {
// 			if (error) {
// 				console.error(error);
// 				alerts.showError('An error has occurred submitting your response. Please try again');
// 			} else {
// 				this.setState({
...
// 	renderGuestsForm = () => {
// 		const { chosenParty } = this.state;
// 		return <GuestsForm guests={chosenParty.guests} updateGuests={this.onUpdateGuests} onCancel={this.goToHome} />;
// 	};

// 	renderConfirmationScreen() {
// 		const { chosenParty } = this.state;
// 		return <Confirmation guests={chosenParty.guests} goToHome={this.goToHome} />;
// 	}
...
// 		return <MultiMatchForm potentialParties={potentialParties} onChooseParty={this.onChooseSingleParty} />;
// 	};
